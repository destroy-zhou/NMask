...
def report(report_config: dict) -> None:
    ...
    if report_config.get("save_path", None) is not None:
        ...
        # --- 新增：提取 MSE 和 MAE 并构建前缀 ---
        try:
            # 假设 leaderboard_df 的结构中包含 'metric_name' 和对应的数值列（通常是最后一列）
            # 我们寻找 mse_norm 和 mae_norm
            metrics = leaderboard_df.set_index(leaderboard_df.columns[1])[leaderboard_df.columns[-1]]
            mse_val = metrics.get("mse_norm", 0)
            mae_val = metrics.get("mae_norm", 0)
            
            # 格式化为 4 位小数的前缀
            name_prefix = f"{mse_val:.4f}_{mae_val:.4f}-"
        except Exception:
            # 如果提取失败（例如列名不对），则不加前缀或加默认前缀
            name_prefix = ""

        # --- 修改：在文件名处加上前缀 ---
        file_name_with_metrics = name_prefix + report_config["leaderboard_file_name"]

        leaderboard_df.to_csv(
            os.path.join(
                ROOT_PATH, "result", save_path, file_name_with_metrics
            ),
            index=False,
        )
    else:

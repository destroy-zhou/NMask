...
datatype_dict = {
    ...
    "Weather": TypePos([], [x for x in range(21)]),
    "Electricity": TypePos([], [x for x in range(321)]),
    "Solar": TypePos([], [x for x in range(137)]),
    "Traffic": TypePos([], [x for x in range(862)]),
    "Exchange": TypePos([], [x for x in range(8)]),
    'Colbun': TypePos([], [x for x in range(3)]),
    'Rapel': TypePos([], [x for x in range(3)]),
    'NP': TypePos([], [x for x in range(3)]),
    'PJM': TypePos([], [x for x in range(3)]),
    'BE': TypePos([], [x for x in range(3)]),
    'FR': TypePos([], [x for x in range(3)]),
    'DE': TypePos([], [x for x in range(3)]),
    'Energy': TypePos([], [x for x in range(6)]),
    'Sdwpfm1': TypePos([], [x for x in range(7)]),
    'Sdwpfm2': TypePos([], [x for x in range(7)]),
    'Sdwpfh1': TypePos([], [x for x in range(7)]),
    'Sdwpfh2': TypePos([], [x for x in range(7)]),
}
...
class TFTTimeFeatureEmbedding(nn.Module):
    ...
    def forward(self, x):
        return torch.stack(
            [embed(x[:, :, i].unsqueeze(-1)) for i, embed in enumerate(self.embed)],
            dim=-2,
        )
...
class TFTEmbedding(nn.Module):
    def __init__(self, configs):
        ...
        if configs.use_future_exog == False:
            ...
            self.cov_know_embedding = nn.ModuleList(
                [DataEmbedding(1, configs.d_model, dropout=configs.dropout) for _ in range(configs.covariate_dim)])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if self.static_len:
            ...
            static_input = torch.stack(
                [
                    embed(x_enc[:, :1, self.static_pos[i]].unsqueeze(-1), None).squeeze(
                        1
                    )
        ...
        cov_x_mark = torch.cat([x_enc[:, :, series_dim:], x_dec], dim=-2)
        x_mark = torch.cat([x_mark_enc, x_mark_dec[:, -self.pred_len :, :]], dim=-2)
        # known_input: [B,T,C,d_model]
        if self.use_future_exog == False:
            known_input = self.known_embedding(x_mark)
        else:
            known_input = torch.stack(
                [embed(cov_x_mark[:, :, i].unsqueeze(-1), None) for i, embed in enumerate(self.cov_know_embedding)],
                dim=-2)
...
class GRN(nn.Module):
    def __init__(
        ...
        self.lin_c = (
            nn.Linear(context_size, hidden_size) if context_size is not None else None
        )
...
class VariableSelectionNetwork(nn.Module):
    def __init__(self, d_model, variable_num, dropout=0.0):
        ...
        self.variable_grns = nn.ModuleList(
            [GRN(d_model, d_model, dropout=dropout) for _ in range(variable_num)]
        )
    ...
    def forward(self, x: Tensor, context: Optional[Tensor] = None):
        ...
        x_processed = torch.stack(
            [grn(x[..., i, :]) for i, grn in enumerate(self.variable_grns)], dim=-1
        )
...
class StaticCovariateEncoder(nn.Module):
    def __init__(self, d_model, static_len, dropout=0.0):
        ...
        self.grns = nn.ModuleList(
            [GRN(d_model, d_model, dropout=dropout) for _ in range(4)]
        )
...
class InterpretableMultiHeadAttention(nn.Module):
    def __init__(self, configs):
        ...
        self.qkv_linears = nn.Linear(
            configs.d_model, (2 * self.n_heads + 1) * self.d_head, bias=False
        )
        ...
        self.register_buffer(
            "mask", torch.triu(torch.full((example_len, example_len), float("-inf")), 1)
        )
...
class TemporalFusionDecoder(nn.Module):
    def __init__(self, configs):
        ...
        self.history_encoder = nn.LSTM(
            configs.d_model, configs.d_model, batch_first=True
        )
        ...
        self.position_wise_grn = GRN(
            configs.d_model, configs.d_model, dropout=configs.dropout
        )
    ...
    def forward(self, history_input, future_input, c_c, c_h, c_e):
        ...
        c = (
            ...
            if c_c is not None and c_h is not None
            else None
        )
        ...
        temporal_features = self.gate_after_lstm(
...
class TemporalFusionTransformer(nn.Module):
    def __init__(self, configs):
        ...
        if self.configs.use_future_exog == False:
            self.known_len = get_known_len(configs.embed, configs.freq)
        else:
            self.known_len = configs.covariate_dim
        ...
        if self.configs.use_future_exog == False:
            self.history_vsn = VariableSelectionNetwork(
                configs.d_model, self.observed_len + self.known_len
            )
        else:
            self.history_vsn = VariableSelectionNetwork(configs.d_model, self.observed_len)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if (
            ...
            series_dim = x_enc.shape[-1] - x_dec.shape[-1]
            x_enc = torch.cat([x_enc[:, :, series_dim:], x_enc[:, :, :series_dim]], dim=-1)
            dec_out = self.forecast(
                x_enc, x_mark_enc, x_dec, x_mark_dec
            )  # [B,pred_len,C]
            dec_out = torch.cat([torch.zeros_like(x_enc), dec_out], dim=1)
            dec_out = torch.cat([dec_out[:, :, -series_dim:], dec_out[:, :, :-series_dim]], dim=-1)

...
from ts_benchmark.baselines.dag_rope.layers.Embed import PatchEmbedding, CompressAndProject
from ts_benchmark.baselines.dag_rope.layers.SelfAttention_Family import FullAttention, AttentionLayer
from ts_benchmark.baselines.dag_rope.layers.Transformer_EncDec import Encoder, EncoderLayer
...
class TemporalCausalityEncoder(nn.Module):
    def __init__(self, enc_in, seq_len, pred_len, series_dim,
                 ...
                 dropout, factor, activation, criterion, use_rope=True
                 ):
    ...
    def forward(self, x, exog_future, use_exog=True):
        ...
        patch_exog, exog_vars = self.exog_patch_embedding(exog_history, add_pe=False)
        patch_x, x_vars = self.x_patch_embedding(x_history, add_pe=False)

...
class TemporalCausalityEncoder(nn.Module):
    def __init__(self, enc_in, seq_len, pred_len, series_dim,
        ...
        if not self.use_future_exog:
            ...
            self.exog_encoder = self._build_exog_encoder(
                d_model=d_model, d_ff=d_ff, n_heads=n_heads, dropout=dropout, activation=activation, output_attention=False,
                factor=factor, e_layers=e_layers, use_rope=use_rope
            )
            ...
            #     nn.Dropout(dropout)
            # )
            d_in = history_patch_num * d_model
            d_out = min(d_in, pred_len) // 2
            # self.exog_head = nn.Sequential(
            #     nn.Linear(d_in, d_out),
            #     nn.ReLU(),
            #     nn.Dropout(dropout),
            #     nn.Linear(d_out, pred_len),
            # )
            # d_in = history_patch_num * (d_model // 2)
            # self.exog_pool = nn.AdaptiveAvgPool1d(d_model // 2)
            self.exog_head = nn.Sequential(
    ...
    def forward(self, x, exog_future, use_exog=True):
        ...
        if self.use_future_exog:
            ...
            # predict exog_future
            exog_in, _ = self.exog_patch_embedding(exog_history)
            x_in, _ = self.exog_patch_embedding(x_history)
            x_in = x_in.view(B, X_D, x_in.shape[-2], x_in.shape[-1])
            exog_in = exog_in.view(B, EXOG_D, exog_in.shape[-2], exog_in.shape[-1])
            # print(f"{patch_x.shape = }, {exog_in.shape = }")
            exog_in = torch.cat([x_in, exog_in], dim=1)
            # print(f"{exog_in.shape = }")
            exog_in = exog_in.view(-1, exog_in.shape[-2], exog_in.shape[-1])
            enc_exog_out, _ = self.exog_encoder(exog_in, self.c_in, exog_attns=None)
            enc_exog_out = torch.reshape(
                enc_exog_out, (B, -1, enc_exog_out.shape[-2], enc_exog_out.shape[-1])
            ) #.permute(0, 1, 3, 2)
            enc_exog_out = enc_exog_out[:,X_D:,:]
            # pooling
            # enc_exog_out = self.exog_pool(enc_exog_out.contiguous().view(-1, enc_exog_out.shape[-2], enc_exog_out.shape[-1]))
            exog_out = enc_exog_out.view(B, EXOG_D, -1)
            # print(f"{exog_out.shape = }")
            ...
            exog_future = exog_out.detach()
            # exog_future_means = exog_future.mean(1, keepdim=True).detach()
            # exog_future_stdev = torch.sqrt(torch.var(exog_future, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
            # exog_future = self.sample_norm(exog_future, exog_future_means, exog_future_stdev)
            exog_future = self.sample_norm(exog_future, exog_history_means, exog_history_stdev)
            exog_future = exog_future.permute(0, 2, 1)
            exog_future, _ = self.x_patch_embedding(exog_future)
            exog_future = exog_future.view(B, EXOG_D, exog_future.shape[-2], exog_future.shape[-1])
            patch_exog = torch.cat([exog_history, exog_future], dim=-2)
    ...
    def _build_exog_encoder(self, d_model, d_ff, n_heads, dropout, activation, output_attention, factor, e_layers, use_rope=False):
        exog_encoder = self._build_encoder(
            d_model=d_model, d_ff=d_ff, n_heads=n_heads, dropout=dropout, activation=activation, output_attention=False,
            factor=factor, e_layers=e_layers, use_rope=use_rope
        )
        return exog_encoder

    ...
    def validate(
        ...
        with torch.no_grad():
            ...
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                # ---- 计时开始 ----
                if self.config.output_efficiency:
                ...
                if self.config.output_efficiency:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    step_end = time.time()
                    train_step_times.append((step_end - step_start) * 1000)  # 转为 ms
        ...
        # output efficiency
        if self.config.output_efficiency:
            avg_train_time = np.mean(train_step_times)
            print(f"Avg Inference Time per Step: {avg_train_time:.2f} ms")
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        if torch.cuda.is_available():
            ...
            print("当前GPU编号:", device_id)
            print("GPU名称:", torch.cuda.get_device_name(device_id))
        else:
            print("没有使用GPU")
        ...
        # max GPU memory
        if torch.cuda.is_available() and self.config.output_efficiency:
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                # kwargs = {}
                # if 'epoch' in inspect.signature(self._process).parameters:
                #     kwargs['epoch'] = epoch
                # out_loss = self._process(
                #     input, target, input_mark, target_mark, exog_future, **kwargs
                # )
                out_loss = self._process(
                    input, target, input_mark, target_mark, exog_future
                )
            ...
            if self.config.output_efficiency:
                avg_train_time = np.mean(train_step_times)
                print(f"Avg Training Time per Step: {avg_train_time:.2f} ms")
                if torch.cuda.is_available():
                    peak_mem = torch.cuda.max_memory_allocated(device) / 1024**2  # MB
                    print(f"Epoch {epoch+1} Peak GPU Memory: {peak_mem:.2f} MB")

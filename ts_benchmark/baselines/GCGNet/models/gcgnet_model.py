...
class GCGNetModel(nn.Module):
    def __init__(self, config):
        ...
        # )
        patch_pred_len = self.patch_len
        # self.head = nn.Sequential(
        #     nn.Linear(self.d_model, patch_pred_len),
        #     nn.Dropout(self.dropout)
        # )
        self.head = PatchHead(
            n_vars=self.var_num,
    ...
    def forward(self, history, exog_future, endo_future=None):
        ...
        # print(f"{patch_endo_enc.shape = }")
        # patch_endo_enc = patch_endo_enc[:, :self.endo_num, self.input_patch_num:]
        # # print(f"{patch_endo_enc.shape = }")
        # patch_endo_enc = self.head(patch_endo_enc)
        # patch_endo_enc = patch_endo_enc.view(patch_endo_enc.shape[0], self.endo_num, -1)

        patch_endo_enc = patch_endo_enc[:, :, self.input_patch_num:]
        patch_endo_enc = self.head(patch_endo_enc)
        endo_target_dec = patch_endo_enc[:,:,:self.pred_len].permute(0, 2, 1)

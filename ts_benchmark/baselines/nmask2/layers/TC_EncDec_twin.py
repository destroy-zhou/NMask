...
class TemporalCausalityEncoder(nn.Module):
    def __init__(self, enc_in, seq_len, pred_len, series_dim,
        ...
        if not self.use_future_exog:
            ...
            #     nn.Dropout(dropout)
            # )
            # pool_num = int(history_patch_num * 0.5)
            # self.exog_pool = nn.AdaptiveMaxPool1d(pool_num)
            # self.exog_head = nn.Sequential(
            #     nn.Linear(pool_num * d_model, pred_len),
            #     nn.Dropout(dropout)
            # )
            self.exog_head = nn.Sequential(
                nn.Linear(history_patch_num * d_model, pred_len),
    ...
    def forward(self, x, exog_future, use_exog=True):
        ...
        if not self.use_future_exog:
            patch_x_nlearn = torch.cat([patch_x, x_future.detach()], dim=-2)
        ...
        if not self.use_future_exog:
            patch_x_in = torch.cat([patch_x_nlearn, patch_exog], dim=1)
        else:
            patch_x_in = torch.cat([patch_x, patch_exog], dim=1)
        
        # print(f"{patch_x_in.shape = }")
        patch_x_in = patch_x_in.view(-1, patch_x_in.shape[-2], patch_x_in.shape[-1])
        if not self.use_rope:
            patch_x_in = patch_x_in + self.position_embedding(patch_x_in)
        ...
        enc_x_out, _ = self.encoder_x(patch_x_in, self.c_in, exog_attns=None)
        ...
        if not self.use_future_exog:
            ...
            exog_out = enc_x_out[:,X_D:,:x_history_L]
            # pooling
            # exog_out = exog_out.contiguous().view(-1, exog_out.shape[-2], exog_out.shape[-1]).permute(0,2,1)
            # exog_out = self.exog_pool(exog_out).permute(0,2,1)
            ...
            exog_in = exog_out.detach()
            exog_in_means = exog_in.mean(1, keepdim=True).detach()
            exog_in_stdev = torch.sqrt(torch.var(exog_in, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
            exog_in = self.sample_norm(exog_in, exog_in_means, exog_in_stdev).permute(0,2,1)

            exog_future, exog_vars = self.x_patch_embedding(exog_in)
            exog_history = exog_history.view(-1, exog_history.shape[-2], exog_history.shape[-1])
            patch_exog = torch.cat([exog_history, exog_future], dim=-2)
            # patch_x, x_vars = self.x_patch_embedding(x_history)
            # patch_x = patch_x.view(B, X_D, patch_x.shape[-2], patch_x.shape[-1])
            # x_history_L = patch_x.shape[-2]
            ...
            # print(f"{patch_x_in.shape = }")
            patch_x_in = patch_x_in.view(-1, patch_x_in.shape[-2], patch_x_in.shape[-1])
            enc_x_out, _ = self.encoder_exg(patch_x_in, self.c_in, exog_attns=None)
            # enc_x_out, _ = self.encoder_x(patch_x_in, self.c_in, exog_attns=None)
            enc_x_out = torch.reshape(
                enc_x_out, (B, -1, enc_x_out.shape[-2], enc_x_out.shape[-1])
            ) 

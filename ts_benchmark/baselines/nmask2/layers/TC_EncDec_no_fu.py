...
class TemporalCausalityEncoder(nn.Module):
    def __init__(self, enc_in, seq_len, pred_len, series_dim,
        ...
        # )
        if not use_future_exog:
            self.encoder_x_no_future = self._build_encoder(
        ...
        if not self.use_future_exog:
            ...
            self.x_head_no_future = nn.Sequential(
                nn.Linear(d_model, patch_pred_len),
                nn.Dropout(dropout)
            )
            self.x_patch_embedding_no_future = PatchEmbedding(
                d_model, patch_len, stride, padding, dropout
            )
            self.x_future_no_future = nn.Parameter(
                # torch.randn(args.num_classes, bottle_dim // 1)
                torch.randn(1, self.series_dim, future_patch_num, d_model)
                # torch.randn(args.num_classes, out_dim * self.c_in)
            )
            self.exog_head_no_future = nn.Sequential(
                nn.Linear(d_model, patch_pred_len),
                nn.Dropout(dropout)
            )
        

    def forward(self, x, exog_future=None, use_exog=True):
        exog_history = x[:, :, self.series_dim:]
        x_history = x[:, :, :self.series_dim]
        x_out, exog_out = None, None
        ...
        if not self.use_future_exog:
            exog_history_means_no_future = exog_history.mean(1, keepdim=True).detach()
            exog_history_stdev_no_future = torch.sqrt(torch.var(exog_history, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
            exog_history_no_future = self.sample_norm(exog_history, exog_history_means_no_future, exog_history_stdev_no_future).permute(0, 2, 1)
        ...
        if not self.use_future_exog:
            # exog_history_no_future, exog_vars = self.x_patch_embedding(exog_history_no_future)
            exog_history_no_future, exog_vars = self.x_patch_embedding_no_future(exog_history_no_future)
            # print(f"{exog_history.shape = }")
            exog_history_no_future = exog_history_no_future.view(B, EXOG_D, exog_history_no_future.shape[-2], exog_history_no_future.shape[-1])
            # x_history_L = patch_x.shape[-2]
            exog_future_tokens = self.exog_future.expand(B, -1, -1, -1)
            patch_exog_no_future = torch.cat([exog_history_no_future, exog_future_tokens], dim=-2)
        ...
        if not self.use_future_exog:
            patch_x_no_future, x_vars = self.x_patch_embedding_no_future(x_history)
            patch_x_no_future = patch_x_no_future.view(B, X_D, patch_x_no_future.shape[-2], patch_x_no_future.shape[-1])
            # x_history_L = patch_x.shape[-2]
            x_future_no_future = self.x_future_no_future.expand(B, -1, -1, -1)
            patch_x_no_future = torch.cat([patch_x_no_future, x_future_no_future], dim=-2)
            patch_x_no_future = torch.cat([patch_x_no_future, patch_exog_no_future], dim=1)
            patch_x_no_future = patch_x_no_future.view(-1, patch_x_no_future.shape[-2], patch_x_no_future.shape[-1])
        if exog_future is not None:
        ...
        if not self.use_future_exog:
            enc_x_out, _ = self.encoder_x_no_future(patch_x_no_future, self.c_in, exog_attns=None)
            enc_x_out = torch.reshape(
                enc_x_out, (B, -1, enc_x_out.shape[-2], enc_x_out.shape[-1])
            )
            enc_x_out = enc_x_out[:,:X_D,x_history_L:]
            x_out_no_future = self.x_head_no_future(enc_x_out)
            x_out_no_future = x_out_no_future.view(B, X_D, -1)
            x_out_no_future = x_out_no_future[:,:,:self.pred_len]

            x_out_no_future = x_out_no_future.permute(0, 2, 1)
            x_out_no_future = self.sample_denorm(x_out_no_future, x_history_means, x_history_stdev)

        if not self.use_future_exog and exog_future is not None:
            exog_out = self.exog_head_no_future(exog_out)
        ...
        if not self.use_future_exog:
            return x_out_no_future, x_out, exog_out

...
from ts_benchmark.baselines.nmask2.layers.Embed import RotaryEmbedding
...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, exog_attn=None, attn_alpha=0.5, tau=None, delta=None):
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / sqrt(E)
        ...
        if self.output_attention:
            return V.contiguous(), A_before_dropout
        else:
            return V.contiguous(), None
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        if use_rope:
            d_keys = self.make_even(d_keys)
            d_values = self.make_even(d_values)
        ...
        if self.use_rope:
            self.rope = RotaryEmbedding(d_keys)

    def make_even(self, x):
